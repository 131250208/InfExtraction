...
def trans2duie2_format(pred_data):
    ...
    for sample_idx, sample in tqdm(enumerate(pred_data), "trans 2 duie2 format"):
        ...
        new_sample = {
            "text": ori_text,
            "spo_list": utils.unique_list(filtered_spo_list),
        }
        new_pred_data.append(new_sample)
...
def convert_duuie():
    ...
    def convert_data(data):
        ...
        def strip_text(dict_):
            if "text" in dict_ and "offset" in dict_:
                l = len(dict_["text"]) - len(dict_["text"].lstrip())
                r = len(dict_["text"]) - len(dict_["text"].rstrip())
                r = len(dict_["offset"]) if r == 0 else -r
                dict_["offset"] = dict_["offset"][l:r]
                dict_["text"] = dict_["text"].strip()
        ...
        for sample in tqdm(data):
            ...
            new_sample = {
                "id": "train_{}".format(len(data_norm)),
                "text": sample["text"],
            }
            ...
            codes = word_tokenizer.tokenize_plus(new_sample["text"], span_list=utils.get_all_possible_char_spans(new_sample),
                                                 tokenize_func="default")  # utils.get_lac("seg").run
            new_sample = {**new_sample, **codes}
            data_norm.append(new_sample)
        return data_norm

    return convert_data(train_data), convert_data(val_data)


if __name__ == "__main__":
    train_data, val_data = convert_duuie()
    save_as_json_lines(train_data, "../../data/normal_data/duuie/train_data.json")
    save_as_json_lines(val_data, "../../data/normal_data/duuie/valid_data.json")
